...
from create import dp, bot, channel_id
from database import sqlite_database
from database.sqlite_database import sql_add_command
import database

from keyboards import kb_client,ikb_client,idb_client,test_kb
...
async def administration_panel(message: types.Message):
    ...
    await bot.send_message(channel_id,f'<code>moderator {aname} has successfully logged in</code>',parse_mode=types.ParseMode.HTML)
...
async def load_description(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await bot.send_message(channel_id,f'<b>updated</b> <code>moderator {aname} added the object to the database</code>',parse_mode=types.ParseMode.HTML)
        await state.finish()

    
#@dp.message_handler(commands=['p'])
async def get_panel_for_moderator(message: types.Message):
    if message.from_user.id == ID:
        await bot.send_message(message.from_user.id,text=f'<code>administation panel</code>\n\nselect an action           ',reply_markup=idb_client,parse_mode=types.ParseMode.HTML)
...
@dp.callback_query_handler(text='delbutton')
async def dell_button(callback: types.CallbackQuery):
...
async def moreinf(call: types.CallbackQuery):
    await call.message.reply_document(open('dihot.txt', 'rb'))
    await call.answer()
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(administration_panel,commands=['apanel'],is_chat_admin = True)
    dp.register_message_handler(get_panel_for_moderator,commands=['p'])
